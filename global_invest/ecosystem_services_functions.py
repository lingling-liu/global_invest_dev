...
recompile_cython = 0
if recompile_cython:
    # NOTE The cython build environment is getting confusing because i'm calling from other projects and thus other CWD
    # Manual compilation works for now.
    old_cwd = os.getcwd()
    os.chdir('global_invest')
    env_name_to_use = sys.prefix
    cython_command = "conda activate " + env_name_to_use + " && " + "python compile_cython_functions.py build_ext -i clean"
